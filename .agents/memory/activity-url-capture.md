...
Activity URLs are optional metadata. Windows desktop agents read an accessible address-bar value through UI Automation for supported browsers, normalize a scheme-less web address to `https://`, and omit non-web values; non-browser apps, unsupported platforms, and inaccessible address bars report no URL. The API and database must continue accepting activity without this field.
