...
**How to apply:** Preserve Active/Passive/Idle overlap correction, per-device timezone bucketing, app ranking, and timeline slots in the compact summary. Keep raw range queries scoped to one selected device, and render an explicit error state rather than substituting zero totals when a summary request fails.